		...
		if (mAMap == null) {
			...
			locationMarker = mAMap.addMarker(new MarkerOptions()
					...
					.icon(BitmapDescriptorFactory
							.fromBitmap(BitmapFactory.decodeResource(getResources(), R.drawable.ic_back_nor)))
	...
	public void onPoiSearched(PoiResult result, int rcode) {
		if (rcode == 1000) {
			if (result != null && result.getQuery() != null) {// 搜索poi的结果
					...
					if (poiItems != null && poiItems.size() > 0) {
						...
						.icon(BitmapDescriptorFactory
								.fromBitmap(BitmapFactory.decodeResource(
										getResources(), R.drawable.ic_back_nor)))
						.position(new LatLng(lp.getLatitude(), lp.getLongitude())));
				...
				ToastUtil
						.show(PoiAroundSearchActivity.this, "无结果");
			}
		}
	}
	...
	private void resetlastmarker() {
		...
		if (index < 10) {
			mlastMarker.setIcon(BitmapDescriptorFactory
					.fromBitmap(BitmapFactory.decodeResource(
							getResources(),
							R.drawable.ic_folder_check)));
		}else {
			mlastMarker.setIcon(BitmapDescriptorFactory.fromBitmap(
			BitmapFactory.decodeResource(getResources(), R.drawable.ic_folder_check)));
		}
		mlastMarker = null;
		...
		protected BitmapDescriptor getBitmapDescriptor(int arg0) {
			if (arg0 < 10) {
				BitmapDescriptor icon = BitmapDescriptorFactory.fromBitmap(
						BitmapFactory.decodeResource(getResources(),R.drawable.ic_delete_photo));
				return icon;
			}else {
				BitmapDescriptor icon = BitmapDescriptorFactory.fromBitmap(
						BitmapFactory.decodeResource(getResources(), R.drawable.ic_delete_photo));
