...
import com.ppbike.R;
import com.ppbike.adapter.RentBikeSubOrderListAdapter;
...
public class RentBikeSubOrderListFragment extends ViewPagerNetworkFragment implements YListView.RefreshTimeListener,YListView.IXListViewListener,   UpdateOrderListener {
	private YListView listView ;
	private LoadView loadView;
	private RentBikeSubOrderListAdapter adapter;
	...
	private void initView(View view){
		...
		adapter = new RentBikeSubOrderListAdapter(this);
		listView.setAdapter(adapter);
		listView.setPullLoadEnable(true);
		listView.setRefreshTimeListener(new YListView.RefreshTimeListener() {
			@Override
			public void onRefreshTime(TextView view) {
				long time = getContext().getSharedPreferences("time", Context.MODE_PRIVATE).getLong(RentBikeSubOrderListFragment.this.getClass().getSimpleName()+type,0l);
	...
	public void onSucceed(String tag, boolean isCache, Object data) {
		...
		if (TAG_REFRESH.equals(tag)){
			...
			getContext().getSharedPreferences("time", Context.MODE_PRIVATE).edit().putLong(RentBikeSubOrderListFragment.this.getClass().getSimpleName()+type,System.currentTimeMillis()).commit();
	...
	public void onRefresh() {
		...
		try {
			...
			RequestModel.obtainRentBikeOrderList(refreshHandler,TAG_REFRESH,body,token);
		} catch (IOException e) {
			e.printStackTrace();
		}
	}
