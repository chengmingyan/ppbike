...
@SuppressLint("NewApi") public class SwitchAnimationUtil {
    ...
    public AnimatorSet startAnimation(View root, AnimationType type) {
...
//	                    bindAnimation(group.getChildAt(i), depth + 1, type);
                    runAnimation(group.getChildAt(i), getmDelay() * mOrderIndex, type);
                    mOrderIndex++;
                }
            }
        } else {
            runAnimation(view, getmDelay() * mOrderIndex, type);
            mOrderIndex++;
        }
    }
    ...
    private void runFlipVertialAnimation(final View view, final long delay) {
        view.setAlpha(0);
        set = new AnimatorSet();
