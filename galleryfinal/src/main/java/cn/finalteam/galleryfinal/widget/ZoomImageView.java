 ...
 * <p/>
 * 注意:此view可继承view,此处是为了需要才继承ImageView
 *
 * @author guolin
 */
public class ZoomImageView extends GFImageView {
    /**
     * 默认状态常量
     */
...
//                }
    ...
    /**
     * 对图片进行初始化操作，包括让图片放大居中，以及当图片大于屏幕宽高时对图片进行压缩。
     *
     * @param canvas
     */
    private void initBitmapByWidth(Canvas canvas) {
        if (sourceBitmap != null) {
            matrix.reset();
            int bitmapWidth = sourceBitmap.getWidth();
            int bitmapHeight = sourceBitmap.getHeight();
